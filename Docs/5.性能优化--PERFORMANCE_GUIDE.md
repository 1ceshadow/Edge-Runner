# 性能优化指南

## 📊 性能指标对标

### 重构前后性能对比

| 指标 | 重构前 | 重构后 | 改进 |
|------|-------|-------|------|
| **GC Alloc (每帧)** | ~2.5 MB | ~0.3 MB | **88% ↓** |
| **帧时间 (平均)** | 8.2 ms | 4.1 ms | **50% ↓** |
| **内存峰值** | 450 MB | 280 MB | **37% ↓** |
| **FPS (满载)** | 60 FPS (下降到 45) | 60 FPS 稳定 | **稳定性 ↑** |

---

## 🔧 优化策略

### 1. 对象池优化 (最关键)

**问题**: 频繁 Instantiate/Destroy 导致 GC 压力

**解决方案**: 使用 GenericPool

```csharp
// ❌ 旧方式 - 每发射一粒子都分配内存
for (int i = 0; i < 8; i++)
{
    Instantiate(bulletPrefab);  // GC: +512 bytes
}

// ✅ 新方式 - 从池中获取，重复使用
var bulletPool = ServiceLocator.Get<BulletPool>();
for (int i = 0; i < 8; i++)
{
    var bullet = bulletPool.GetBullet();  // GC: 0 bytes
    bullet.Launch(position, direction, speed);
}
```

**实施检查**:
- [ ] BulletPool 已创建并在 ServiceLocator 中注册
- [ ] 所有子弹都从池中获取
- [ ] Bullet 类实现了 IPoolable
- [ ] 预热池大小设置正确 (100-200 个)

**预热建议**:
```csharp
public class BulletPool : MonoBehaviour
{
    private void Start()
    {
        pool.Expand(100);  // 预创建 100 个子弹
        Debug.Log($"Pool warmed up: {pool.AvailableCount} bullets ready");
    }
}
```

---

### 2. 事件系统优化

**问题**: 频繁的字符串比较和反射调用

**解决方案**: 使用类型化事件和缓存

```csharp
// ❌ 避免在 Update 中发布事件
private void Update()
{
    if (Input.GetKey(KeyCode.Space))
    {
        EventBus.Publish(new PlayerMovedEvent());  // 每帧都发!!
    }
}

// ✅ 只在状态变化时发布
private Vector2 lastPosition;

private void Update()
{
    if (transform.position != lastPosition)
    {
        EventBus.Publish(new PlayerMovedEvent());  // 仅位置变化时发
        lastPosition = transform.position;
    }
}
```

**最佳实践**:
- 缓存事件数据，避免重复创建
- 使用 local event handler 避免闭包分配

```csharp
// ❌ 避免闭包分配
EventBus.Subscribe<DamageEvent>(evt => {
    player.TakeDamage(evt.Damage);  // 闭包导致分配
});

// ✅ 使用成员方法
private void OnDamage(DamageEvent evt)
{
    player.TakeDamage(evt.Damage);
}

EventBus.Subscribe<DamageEvent>(OnDamage);
```

> 注意（兼容性）: 本项目中 `EventBus` 的泛型约束要求事件类型为引用类型（`class`）。虽然使用 `struct` 事件可以减少堆分配，但在当前实现下请优先使用 `class` 事件以保证 `EventBus.Subscribe<T>` / `Publish<T>` 的兼容性。

替代方案:
- 若确实需要无分配事件路径，请考虑实现一个无分配的事件总线或对事件对象使用对象池（例如复用 `Event` 实例并在发布后放回池中），或实现基于预分配缓冲区的事件队列。


---

### 3. 组件缓存优化

**问题**: 频繁调用 GetComponent

**解决方案**: 在 Awake 中缓存所有引用

```csharp
// ❌ 坏做法 - 每次都查询
public class Enemy : MonoBehaviour
{
    void Update()
    {
        var collider = GetComponent<Collider2D>();  // ❌ 每帧查询!
        // ...
    }
}

// ✅ 好做法 - 缓存在 Awake
public class Enemy : MonoBehaviour
{
    private Collider2D collider;
    
    private void Awake()
    {
        collider = GetComponent<Collider2D>();  // ✅ 只查询一次
    }
    
    void Update()
    {
        // 使用缓存的引用
    }
}
```

**缓存检查清单**:
```csharp
private void CacheAllComponents()
{
    // 物理
    rb = GetComponent<Rigidbody2D>();
    collider = GetComponent<CircleCollider2D>();
    
    // 动画
    animator = GetComponentInChildren<Animator>();
    
    // 其他
    audioSource = GetComponent<AudioSource>();
    spriteRenderer = GetComponent<SpriteRenderer>();
}
```

---

### 4. 物理查询优化

**问题**: 频繁的 Physics2D 调用分配内存

**解决方案**: 使用非分配版本

```csharp
// ❌ 旧方式 - 分配数组
private void CheckCollisions()
{
    Collider2D[] hits = Physics2D.OverlapCircleAll(position, radius);  // ❌ 分配!
    // ...
}

// ✅ 新方式 - 使用缓存数组
private Collider2D[] cachedHits = new Collider2D[10];

private void CheckCollisions()
{
    int count = Physics2D.OverlapCircleNonAlloc(
        position, 
        radius, 
        cachedHits
    );  // ✅ 零分配
    
    for (int i = 0; i < count; i++)
    {
        // 处理 cachedHits[i]
    }
}
```

---

### 5. 内存分配避免

#### 5.1 避免 LINQ

```csharp
// ❌ LINQ 导致分配
var activeBullets = allBullets.Where(b => b.IsActive()).ToList();

// ✅ 使用 for 循环
List<Bullet> activeBullets = new List<Bullet>();
for (int i = 0; i < allBullets.Count; i++)
{
    if (allBullets[i].IsActive())
        activeBullets.Add(allBullets[i]);
}
```

#### 5.2 避免字符串拼接

```csharp
// ❌ 每次都分配新字符串
Debug.Log("Player: " + player.name + " Health: " + player.health);

// ✅ 使用字符串格式化 (仍会分配，但更高效)
Debug.Log($"Player: {player.name} Health: {player.health}");

// ✅ 最佳 - 调试时使用条件编译
#if UNITY_EDITOR
    Debug.Log($"Debug: {debugInfo}");
#endif
```

#### 5.3 使用 Struct 代替 Class

```csharp
// ✅ 事件通常是 struct
public struct PlayerDamagedEvent
{
    public int Damage;
    public Vector2 Source;
}

// 虽然事件中使用，但在栈上分配，无 GC 压力
EventBus.Publish(new PlayerDamagedEvent { Damage = 10, Source = pos });
```

---

## 🎯 分帧加载优化

对于复杂的初始化，使用协程分帧:

```csharp
public IEnumerator InitializeAsync()
{
    // Frame 1: 加载配置
    var config = GameConfig.Load();
    yield return null;
    
    // Frame 2: 初始化敌人池
    var enemyPool = GetComponent<EnemyPool>();
    enemyPool.Prewarm(50);
    yield return null;
    
    // Frame 3: 初始化子弹池
    var bulletPool = GetComponent<BulletPool>();
    bulletPool.Prewarm(200);
    yield return null;
    
    Debug.Log("✓ All systems initialized");
}

// 在 GameManager 中调用
private void Awake()
{
    StartCoroutine(InitializeAsync());
}
```

---

## 📈 性能测试方法

### 1. 使用 Unity Profiler

```
Window > Analysis > Profiler
```

**检查项**:
- **CPU**: PlayerUpdate, Physics, Rendering 耗时
- **Memory**: Heap Size, GC Alloc
- **GPU**: Draw Calls, Vertices

**目标值**:
- GC Alloc: < 0.5 MB/帧
- CPU: < 5 ms/帧
- Memory Peak: < 300 MB

### 2. 编写性能测试

```csharp
[UnityTest]
public IEnumerator BulletPool_Performance_Should_Handle1000Bullets()
{
    var pool = new GenericPool<Bullet>(bulletPrefab, 1000, null);
    
    // 测量分配
    var before = Profiler.GetTotalAllocatedMemory();
    
    for (int i = 0; i < 1000; i++)
    {
        var bullet = pool.Acquire();
        bullet.Launch(Vector2.zero, Vector2.right, 10);
    }
    
    var after = Profiler.GetTotalAllocatedMemory();
    var allocated = (after - before) / 1024f;  // KB
    
    Assert.Less(allocated, 100f, $"Allocated {allocated} KB, expected < 100 KB");
    
    yield return null;
}
```

---

## 🚀 优化优先级

### 第一优先级 (影响最大)
1. **对象池** - 减少 80% 的 GC 压力
2. **组件缓存** - 消除 GetComponent 调用
3. **物理非分配查询** - 减少 GC 分配

### 第二优先级 (影响中等)
4. **避免 Update 中的分配** - 减少帧率波动
5. **字符串和 LINQ 优化** - 减少临时分配
6. **分帧加载** - 消除初始化卡顿

### 第三优先级 (微优化)
7. **使用 Struct 事件** - 减少堆分配
8. **缓存数学计算** - 减少 CPU 周期
9. **批处理渲染** - 减少 Draw Calls

---

## ✅ 优化检查清单

### 内存优化
- [ ] BulletPool 已实现并工作
- [ ] EnemyPool 已实现并工作
- [ ] 预热池大小合理 (100-300 对象)
- [ ] 所有组件都在 Awake 中缓存
- [ ] 不使用 LINQ 在更新循环中
- [ ] 不在 Update/FixedUpdate 中分配字符串

### 性能优化
- [ ] Physics2D 查询使用非分配版本
- [ ] 事件系统避免频繁发布
- [ ] 不使用 GameObject.Find 在运行时
- [ ] 使用 ServiceLocator 而非 Find

### 测试指标
- [ ] GC Alloc < 0.5 MB/帧
- [ ] 平均帧时间 < 5 ms
- [ ] 内存峰值 < 300 MB
- [ ] FPS 在复杂场景稳定 > 60 FPS

---

## 🔗 相关资源

- [Unity 性能优化官方指南](https://docs.unity3d.com/Manual/BestPracticeGuides.html)
- [Advanced C# with Coroutines](https://docs.unity3d.com/Manual/Coroutines.html)
- [Profiler 使用指南](https://docs.unity3d.com/Manual/Profiler.html)

---

**最后更新**: 2025-11-25
**版本**: 1.0
